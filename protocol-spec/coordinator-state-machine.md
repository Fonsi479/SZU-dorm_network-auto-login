...
5. Provider 开关关闭则不认证。
6. 通常只有明确 `offline` 才能进入凭据授权。Dorm 自动恢复另有一个收窄例外：
   直连外网失败、在线列表明确缺少本机精确记录、设备数已知且小于 3。
...
- 密码错误、账号不存在、欠费、产品无效：不自动重试。
- 设备超限阻止当前操作，但不是永久 fatal；下一轮重新读取设备数，只有从 3/3 变为低于上限后才可继续。
