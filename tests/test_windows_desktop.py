...
class WindowsAppLifecycleTests(unittest.TestCase):
    def test_status_refresh_skips_default_internet_until_portal_online(self) -> None:
        ...
        app.logger = Mock()
        app.service = Mock()
        app.service.status.return_value = {
        ...
        app.service.status.assert_called_once_with()
    ...
    def test_frozen_control_action_captures_output_for_windowed_gui(self) -> None:
        ...
        with patch("src.szu_netlogin.control.main", side_effect=fake_main):
            result = run_frozen_control_action(["status"], service=service)
    ...
    def test_teaching_watchdog_requires_one_verified_enabled_provider(self) -> None:
        teaching = DesktopStatusResult(
            ...
            environment_label="teaching",
            auto_login_available=True,
            portal_session_state="offline",
            teaching_enabled=True,
            teaching_session_state="offline",
            auto_login_provider="teaching",
        )
        ambiguous = DesktopStatusResult(
            False,
            NetworkStatus(False, False),
            environment_label="ambiguous",
            auto_login_available=False,
            portal_session_state="offline",
            auto_login_provider=None,
        )
        self.assertTrue(should_start_auto_login(False, teaching))
        self.assertFalse(should_start_auto_login(False, ambiguous))
    ...
    def test_close_cancels_process_callbacks_and_window(self) -> None:
        ...
        app.service.cancel_pending_operations.assert_called_once_with()
        self.assertEqual(
