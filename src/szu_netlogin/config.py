...
PROVIDER_IDS = ("dorm", "teaching")
_CREDENTIAL_REF_RE = re.compile(r"[A-Za-z0-9._:-]{1,160}")
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if os.name == "nt" and password_source != "keychain":
        raise ConfigError("Windows 只允许 Credential Manager/keychain 密码来源。")

    general = config.get("general")
    if general is not None:
        if not isinstance(general, dict):
            raise ConfigError("general 必须是配置段。")
        for key in ("autoDetect", "connectivityProbe", "launchAtLogin", "paused"):
            if key in general and not isinstance(general[key], bool):
                raise ConfigError(f"general.{key} 必须是布尔值。")

    providers = config.get("providers")
    if providers is not None:
        if not isinstance(providers, dict):
            raise ConfigError("providers 必须是配置段。")
        for provider_id in PROVIDER_IDS:
            provider = providers.get(provider_id)
            if not isinstance(provider, dict):
                raise ConfigError(f"providers.{provider_id} 必须是配置段。")
            if not isinstance(provider.get("enabled"), bool):
                raise ConfigError(f"providers.{provider_id}.enabled 必须是布尔值。")
            account = str(provider.get("accountLabel") or "")
            if len(account) > 128:
                raise ConfigError(f"providers.{provider_id}.accountLabel 过长。")
            credential_ref = str(provider.get("credentialRef") or "")
            if not _CREDENTIAL_REF_RE.fullmatch(credential_ref):
                raise ConfigError(f"providers.{provider_id}.credentialRef 格式无效。")
        teaching = providers["teaching"]
        suffix_mode = str(teaching.get("accountSuffixMode") or "auto")
        if suffix_mode not in {"auto", "none", "hlw", "xyw", "custom"}:
            raise ConfigError("providers.teaching.account_suffix_mode 无效。")
        if len(str(teaching.get("customSuffix") or "")) > 32:
            raise ConfigError("providers.teaching.customSuffix 过长。")

    portal_host = str(network.get("portalHost") or "net.szu.edu.cn")
    if portal_host != "net.szu.edu.cn":
        raise ConfigError("network.portal_host 必须是 net.szu.edu.cn。")


def provider_configuration(config: dict[str, Any], provider_id: str) -> dict[str, Any]:
    if provider_id not in PROVIDER_IDS:
        raise ConfigError("provider 必须是 dorm 或 teaching。")
    providers = config.get("providers") or {}
    configured = providers.get(provider_id) or {}
    legacy_user = str((config.get("user") or {}).get("username") or "").strip()
    account = str(configured.get("accountLabel") or "").strip()
    if not account or account == "你的校园卡号，不要写密码":
        account = legacy_user
    security = config.get("security") or {}
    default_ref = str(security.get("keychain_account") or account or f"provider:{provider_id}")
    return {
        "enabled": bool(configured.get("enabled", provider_id == "dorm")),
        "account_label": account,
        "credential_ref": str(configured.get("credentialRef") or default_ref),
        "account_suffix_mode": str(configured.get("accountSuffixMode") or "auto"),
        "custom_suffix": str(configured.get("customSuffix") or ""),
    }


def teaching_account(config: dict[str, Any]) -> str:
    provider = provider_configuration(config, "teaching")
    account = provider["account_label"]
    mode = provider["account_suffix_mode"]
    if not account or "@" in account or mode in {"auto", "none"}:
        return account
    suffix = provider["custom_suffix"] if mode == "custom" else mode
