...
def has_password(config_or_username: dict[str, Any] | str) -> bool:
    ...
    username = config_or_username
    if os.name == "nt":
        return bool(_get_keyring_password(SERVICE_NAME, username))
...
def get_provider_password(config: dict[str, Any], provider_id: str) -> str:
    provider = provider_configuration(config, provider_id)
    security = config.get("security") or {}
    return _get_keyring_password(
        str(security.get("keychain_service") or SERVICE_NAME),
        provider["credential_ref"],
    )


def set_provider_password(config: dict[str, Any], provider_id: str, password: str) -> None:
    if not password:
...
def credential_backend_status() -> tuple[bool, str]:
    if os.name != "nt":
        return True, "non-windows-development"
    try:
        _load_keyring()
    except Exception:
        return False, "windows-credential-manager-unavailable"
...
def _load_keyring():
    ...
    if os.name == "nt":
        ...
        if "windows" not in backend_name:
            raise RuntimeError("Windows Credential Manager backend is required")
    return keyring
