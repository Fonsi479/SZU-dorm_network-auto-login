...
def main(
    ...
    *,
    ...
    product_service = service or get_process_service()
    ...
    if args.command == "status":
        return print_status(product_service)
    ...
    if args.command == "logout":
        return logout_now(args.pause_for, service=product_service)

    if args.command == "set-username":
        return set_username(args.username)

    if args.command == "set-password":
        print("命令行不接收密码；请在 Windows 桌面客户端中保存到 Credential Manager。")
        return 2

    if args.command == "set-provider-enabled":
        try:
            set_provider_enabled(args.provider, args.enabled == "true")
...
def print_status(service: WindowsCampusService | None = None) -> int:
    if sys.platform != "darwin":
        try:
            status = (service or get_process_service()).status()
        except ConfigError as exc:
            print(f"配置提示：{exc}")
            return 2
        print("SZU Campus Network 状态")
        print(f"网络类别：{status['networkContext']}")
...
def login_now(service: WindowsCampusService | None = None) -> int:
...
def _run_product_login(
    *, manual: bool, service: WindowsCampusService | None = None
) -> int:
    try:
        result = (service or get_process_service()).login("auto", manual=manual)
    except ConfigError as exc:
        print(f"配置检查失败：{exc}")
        return 2
...
def logout_now(
    pause_for: str = "manual", *, service: WindowsCampusService | None = None
) -> int:
    from .dorm_drcom_client import DormDrcomClient
    ...
    if sys.platform != "darwin":
        result = (service or get_process_service()).logout("auto")
        print(f"退出结果：{result.outcome.value} provider={result.provider_id} code={result.error_code or '-'}")
        return 0 if result.outcome.value in {"succeeded", "unchanged"} else 2
