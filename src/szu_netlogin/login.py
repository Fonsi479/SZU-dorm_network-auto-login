...
def main() -> int:
    ...
    try:
        ...
        print(f"已检查宿舍区网关 {gateway_label}:801...")
        if not network_status.gateway_reachable:
            environment = classify_network_environment(config, network_status)
            reason_label = login_failure_reason_label(_network_login_skip_reason(network_status.gateway_reason))
            print(f"当前网络环境：{environment.label}")
            print(f"登录跳过原因：{reason_label}。")
            logger.info("登录停止本轮：%s", reason_label)
            return 0

        environment = classify_network_environment(config, network_status)
        if not environment.auto_login_available:
            print(f"当前网络环境：{environment.label}")
            print("登录跳过原因：源路由未验证，拒绝读取或发送账号密码。")
            logger.warning("登录停止本轮：source route unverified")
            return 0
        if args.check_and_login and is_paused():
            print("当前已暂停，跳过自动登录")
        ...
        print("门户已确认离线，准备尝试宿舍区 Dr.COM 登录。")
        logger.info("门户已确认离线，尝试登录")
        ...
        result = client.login_with_result(
            username,
            password,
            known_source_ip=network_status.source_ip,
        )
