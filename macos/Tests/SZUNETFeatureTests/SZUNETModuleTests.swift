...
struct SZUNETModuleTests {
    ...
    func disabledAdapterIsPassive() async {
        ...
        _ = await module.setNetworkProbeEnabled(false)
        _ = await module.setProbeInterval(.twoMinutes)
    ...
    func actionsRemainHighLevel() async {
        ...
        #expect(commands.map(\.command) == [
            .login, .logout, .pause, .resume, .disableProbe,
            .probeEvery120Seconds, .openSettings, .diagnostics,
        ])
    ...
    @Test("disabling during startup always clears the working state")
    @MainActor
    func disablingStoreClearsStartupWork() async {
        let executor = BlockingCommandExecutor()
        let store = SZUNETFeatureStore(
            module: SZUNETModule(executor: executor),
            refreshPolicy: SZUNETRefreshPolicy(
        ...
        #expect(!store.isWorking)
        #expect(!store.snapshot.adapterEnabled)
        await store.shutdown()
    }
